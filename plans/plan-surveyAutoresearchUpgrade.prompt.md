...
- Phase 4 inference implementation now includes decoder JSON decoding modes in `inference.py`.
- Default decoder mode is schema-constrained JSON decoding (`--decoder-json-decoding constrained`) backed by Outlines when available.
- A compatibility fallback mode (`--decoder-json-decoding legacy`) keeps the previous free-form generate -> parse behavior for debugging and environments without Outlines.
- Encoder deterministic JSON assembly path is unchanged and remains the schema-stable baseline.
...
- Decoder JSON strategy: use schema-constrained JSON decoding in inference by default (`--decoder-json-decoding constrained`) and retain a legacy fallback mode for troubleshooting.
